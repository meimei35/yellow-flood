...
static const float3 NightAmbientMapPosY = float3(0.05, 0.05, 0.05);  // kills everything
...
static const float CITY_LIGHTS_INTENSITY 		= 3.0;
static const float CITY_LIGHTS_BLOOM_FACTOR 	= 0.8;
...
static const float 	SNOW_CAM_MAX 				= 500.0f;
...
static const float WATER_COLOR_LIGHTNESS = 1.0; // 0.5
static const float WATER_RIPPLE_EFFECT = 0.0025;

static const float COLORMAP_OVERLAY_STRENGTH 	= 0.75f; //0.6f; //0.75
...
static const float GB_CAM_MAX = 500.0f; // 1.0
static const float GB_CAM_MAX_FILLING_CLAMP = 0.95f; // 0 to 1 value for clamping the fill when camera is at max distance
static const float GB_THRESHOLD = 0.05f; // interpolation time // 0.01 // 0.1
static const float GB_THRESHOLD2 = 0.25f; // interpolation time // 0.15 // 0.65
...
static const float GB_OPACITY_NEAR = 0.99f; // Transparency when camera is near // 1.0
static const float GB_OPACITY_FAR = 1.0f;  // Transparency when camera is far
static const float BORDER_NIGHT_DESATURATION_MAX = 0.7f; // how much border colors can get desaturated at night. 1.0f is full grey
static const float BORDER_FOW_REMOVAL_FACTOR = 1.0f; // How much of the FOW that is removed from the borders. 1.0f is no FOW
static const float BORDER_LIGHT_REMOVAL_FACTOR = 0.99f; // How much of the light calculations that are removed from the borders. 1.0f is no light // 0.8 //0.99
static const float GB_STRENGTH_CH1 = 1.0; // Opacity of bottom layer // 0.8
static const float GB_STRENGTH_CH2 = 0.4; // Opacity of top layer // 0.8
static const float GB_FIRST_LAYER_PRIORITY = 0.4; // Priority for first/second layer when both are active at the same pixel // 0.4
